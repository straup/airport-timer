/*
 * this is a patched version of https://github.com/qhoxie/jquery.stopwatch
 * see the incrementer() function for details (20111129/straup)
*/

(function( $ ){

    /* 
     * this is not ideal but necessary to account for the fact
     * the default incrementer blocks (read: stops) when safari
     * is background-ed on the iphone or the phone goes to sleep
     */
    ...
    function _original_incrementer(ct, increment) {
    ...
    var methods = {
        ...
        init: function(options) {
		    ...
		    // see notes above in incrementer()
		    // data.incrementer = incrementer(data.startTime, data.updateInterval);

                    data.incrementer = incrementer();

                    data.tick_function = function() {
                        var millis = data.incrementer(data.startTime);
                        data.elapsed = millis;
